...
#Filtering to removing bleach synched K04F10.3 animals as they showed similar results as the egg laying synchornizatin prepared K04F10.3 animals and for consistency becuase the controls were prepared by standard egg laying synchronization. 

cleanrawsmorph <- cleanrawsmorph %>% 
  filter(genotype %in% c("PD1074", "pbs1", "K04F10S"))
...
ggplot(mean_cleanrawsrev_filt, aes(stimulus_num, rev_speed, colour = genotype)) +
  ...
  scale_y_continuous(expand = c(0,0),
                     limits = c(0,0.4), breaks = c(0,0.1,0.2,0.3)) +
  scale_x_continuous(expand = c(0,0),
                     limits = c(0,32), breaks = c(0,31)) +
  geom_point(size = 0.8) +
  scale_color_manual(values=c("black", "dodgerblue2", "gray39")) +
...
data <- read.table("/Users/troymcdiarmid/Desktop/laptop_transfer/peel1_dels_0306_2020/data/data.srev", sep="", header = FALSE)
